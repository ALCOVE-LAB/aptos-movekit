...
module movekit::access_control_admin_registry_tests {
    ...
    fun test_init_module_success(deployer: &signer) {
        ...
        assert!(access_control_admin_registry::get_current_admin() == deployer_addr, 0);
        assert!(access_control_admin_registry::is_current_admin(deployer_addr), 1);

        // No pending admin initially
        assert!(!access_control_admin_registry::has_pending_admin(deployer_addr), 2);
    }
    ...
    fun test_is_current_admin_various_addresses(deployer: &signer) {
        ...
        assert!(access_control_admin_registry::is_current_admin(deployer_addr), 0);
        assert!(!access_control_admin_registry::is_current_admin(@0x123), 1);
        assert!(!access_control_admin_registry::is_current_admin(@0x456), 2);
    }
    ...
    fun test_transfer_admin_complete_flow(
        ...
        // Check pending state
        assert!(access_control_admin_registry::has_pending_admin(admin_addr), 0);
        assert!(access_control_admin_registry::get_pending_admin() == new_admin_addr, 1);

        // Admin should still be current
        assert!(access_control_admin_registry::get_current_admin() == admin_addr, 2);
        assert!(access_control_admin_registry::is_current_admin(admin_addr), 3);
        assert!(!access_control_admin_registry::is_current_admin(new_admin_addr), 4);

        // Step 2: Accept transfer
        access_control_admin_registry::accept_pending_admin(new_admin);

        // Check transfer completed
        assert!(access_control_admin_registry::get_current_admin() == new_admin_addr, 5);
        assert!(access_control_admin_registry::is_current_admin(new_admin_addr), 6);
        assert!(!access_control_admin_registry::is_current_admin(admin_addr), 7);

        // Check pending admin cleaned up
        assert!(!access_control_admin_registry::has_pending_admin(admin_addr), 8);
    }
    ...
    fun test_transfer_admin_overwrite_proposal(
        ...
        // First proposal
        access_control_admin_registry::transfer_admin(admin, new_admin_addr);
        assert!(access_control_admin_registry::get_pending_admin() == new_admin_addr, 0);

        // Second proposal overwrites first
        access_control_admin_registry::transfer_admin(admin, another_addr);
        assert!(access_control_admin_registry::get_pending_admin() == another_addr, 1);

        // Old proposal is no longer valid
        assert!(access_control_admin_registry::get_pending_admin() != new_admin_addr, 2);
    }
    ...
    fun test_cancel_admin_transfer(admin: &signer, new_admin: &signer) {
        ...
        assert!(access_control_admin_registry::get_current_admin() == admin_addr, 1);
        assert!(access_control_admin_registry::is_current_admin(admin_addr), 2);

        // Check pending admin cleaned up
        assert!(!access_control_admin_registry::has_pending_admin(admin_addr), 3);
    }
    ...
    fun test_transfer_admin_not_admin(admin: &signer, non_admin: &signer) {
        access_control_admin_registry::init_for_testing(admin);
    ...
    fun test_transfer_chain(
        ...
        assert!(access_control_admin_registry::get_current_admin() == new_admin1_addr, 0);
        assert!(!access_control_admin_registry::is_current_admin(admin_addr), 1);
        ...
        assert!(access_control_admin_registry::get_current_admin() == new_admin2_addr, 2);
        assert!(!access_control_admin_registry::is_current_admin(new_admin1_addr), 3);
        assert!(!access_control_admin_registry::is_current_admin(admin_addr), 4);
    }
    ...
    fun test_transfer_to_zero_address(admin: &signer) {
        ...
        let admin_addr = signer::address_of(admin);
        assert!(access_control_admin_registry::has_pending_admin(admin_addr), 0);
        assert!(access_control_admin_registry::get_pending_admin() == @0x0, 1);
    }
    ...
    fun test_has_pending_admin_various_states(admin: &signer) {
        ...
        assert!(access_control_admin_registry::has_pending_admin(admin_addr), 1);

        // After canceling
        access_control_admin_registry::cancel_admin_transfer(admin);
        assert!(!access_control_admin_registry::has_pending_admin(admin_addr), 2);
    }

    #[test]
    fun test_has_pending_admin_nonexistent_address() {
        // Should return false for addresses that never had pending admin
        assert!(!access_control_admin_registry::has_pending_admin(@0x123), 0);
        assert!(!access_control_admin_registry::has_pending_admin(@0x456), 1);
    }
    ...
    fun test_multiple_proposal_overwrites(admin: &signer) {
        ...
        assert!(access_control_admin_registry::get_pending_admin() == @0x111, 0);

        access_control_admin_registry::transfer_admin(admin, @0x222);
        assert!(access_control_admin_registry::get_pending_admin() == @0x222, 1);

        access_control_admin_registry::transfer_admin(admin, @0x333);
        assert!(access_control_admin_registry::get_pending_admin() == @0x333, 2);

        // Only the last proposal is valid
        assert!(access_control_admin_registry::get_pending_admin() != @0x111, 3);
        assert!(access_control_admin_registry::get_pending_admin() != @0x222, 4);
    }
    ...
    fun test_pending_admin_cleanup_after_accept(
        ...
        assert!(!access_control_admin_registry::has_pending_admin(new_admin_addr), 1);
    }
    ...
    fun test_no_pending_admin_leaks_after_accept(
        ...
        assert!(!access_control_admin_registry::has_pending_admin(admin_addr), 1);
        assert!(!access_control_admin_registry::has_pending_admin(new_admin_addr), 2);
        assert!(!access_control_admin_registry::has_pending_admin(@movekit), 3);

        // Test random addresses too
        assert!(!access_control_admin_registry::has_pending_admin(@0x999), 4);
        assert!(!access_control_admin_registry::has_pending_admin(@0x888), 5);
    }
    ...
    fun test_no_pending_admin_leaks_after_cancel(admin: &signer) {
        ...
        // Verify NO pending admin resources exist anywhere
        assert!(!access_control_admin_registry::has_pending_admin(admin_addr), 1);
        assert!(!access_control_admin_registry::has_pending_admin(@0x123), 2);
        assert!(!access_control_admin_registry::has_pending_admin(@movekit), 3);
    }
    ...
    fun test_no_pending_admin_leaks_after_overwrite(admin: &signer) {
        ...
        assert!(access_control_admin_registry::has_pending_admin(admin_addr), 0);
        assert!(access_control_admin_registry::get_pending_admin() == @0x333, 1);
    ...
    fun test_propose_cancel_propose_sequence(
        ...
        assert!(access_control_admin_registry::get_current_admin() == new_admin_addr, 0);
        assert!(!access_control_admin_registry::has_pending_admin(admin_addr), 1);
    }
    ...
    fun test_interleaved_operations(admin: &signer, new_admin: &signer) {
        ...
        // Verify final state
        assert!(access_control_admin_registry::get_current_admin() == new_admin_addr, 2);
    }
    ...
    fun test_view_functions_consistency_under_state_changes(
        ...
        assert!(access_control_admin_registry::is_current_admin(admin_addr), 0);
        assert!(access_control_admin_registry::get_current_admin() == admin_addr, 1);
        assert!(!access_control_admin_registry::has_pending_admin(admin_addr), 2);

        // After proposal
        access_control_admin_registry::transfer_admin(admin, @0x123);
        assert!(access_control_admin_registry::is_current_admin(admin_addr), 3); // Still current
        assert!(access_control_admin_registry::get_current_admin() == admin_addr, 4); // Still current
        assert!(access_control_admin_registry::has_pending_admin(admin_addr), 5); // Now has pending

        // After cancel
        access_control_admin_registry::cancel_admin_transfer(admin);
        assert!(access_control_admin_registry::is_current_admin(admin_addr), 6); // Still current
        assert!(access_control_admin_registry::get_current_admin() == admin_addr, 7); // Still current
        assert!(!access_control_admin_registry::has_pending_admin(admin_addr), 8); // No longer pending
    }
    ...
    fun test_resource_existence_consistency(admin: &signer) {
        ...
        access_control_admin_registry::transfer_admin(admin, @0x123);
        assert!(access_control_admin_registry::has_pending_admin(admin_addr), 4);

        access_control_admin_registry::cancel_admin_transfer(admin);
        assert!(!access_control_admin_registry::has_pending_admin(admin_addr), 5);
    }
    ...
    fun test_admin_uniqueness_invariant(
        ...
        assert!(access_control_admin_registry::is_current_admin(admin_addr), 0);
        assert!(!access_control_admin_registry::is_current_admin(new_admin_addr), 1);

        // During proposal: admin still admin, new_admin still not
        access_control_admin_registry::transfer_admin(admin, new_admin_addr);
        assert!(access_control_admin_registry::is_current_admin(admin_addr), 2);
        assert!(!access_control_admin_registry::is_current_admin(new_admin_addr), 3);

        // After transfer: new_admin is admin, admin is not
        access_control_admin_registry::accept_pending_admin(new_admin);
        assert!(!access_control_admin_registry::is_current_admin(admin_addr), 4);
        assert!(access_control_admin_registry::is_current_admin(new_admin_addr), 5);

        // Verify no other addresses are admin
        assert!(!access_control_admin_registry::is_current_admin(@0x999), 6);
        assert!(!access_control_admin_registry::is_current_admin(@movekit), 7);
    }
    ...
    fun test_state_machine_invariants(admin: &signer) {
        ...
        assert!(access_control_admin_registry::get_current_admin() == admin_addr, 0);
        ...
        assert!(current == admin_addr, 4); // Should be a concrete address, not null
    }
}
